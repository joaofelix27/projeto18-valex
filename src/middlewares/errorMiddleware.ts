...
export default function errorHandler (error:any,req:Request,res:Response,next:NextFunction) {
    ...
    if (error.type === "error_block") return res.status(409).send({ message: error.message});
    if (error.type === "error_business") return res.status(404).send({ message: error.message});
    if (error.type === "error_purchase") return res.status(404).send({ message: error.message});
